...
**Impl-agent dispatch.** Impl agents (those dispatched to write code, run tests, and/or commit changes by `/fix-issues` PR mode, `/do` PR mode, `/land-pr`'s fix-cycle template, `/run-plan`, and `/quickfix`'s agent-dispatched mode) MUST be dispatched with `subagent_type: "fixer"`. The fixer agent (`.claude/agents/fixer.md`) clones verifier's frontmatter hook (`inject-bash-timeout.sh`), so its Bash calls auto-extend to a 600s timeout. This prevents the bg+Monitor stall pattern that would otherwise fire when a long test suite exceeds the Bash tool's default 120s timeout — symmetric to the Layer 0 protection that verifier dispatches already get. Conformance tripwires in `tests/test-skill-conformance.sh` (section "fixer subagent — impl-dispatch site pins") assert each impl-dispatch site declares `subagent_type: "fixer"`; the assertion fails closed if a future edit drops the directive.
