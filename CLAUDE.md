...
- `agents/` — plugin-native verifier/implementer subagent definitions (branch 1A): body-identical to `.claude/agents/{verifier,implementer}.md` minus the frontmatter `hooks:` block (plugin agents ignore frontmatter hooks; Layer-0 arrives via `hooks/hooks.json` instead — see Verifier-cannot-run rule). Parity locked by `tests/test-agents-parity.sh`.
...
- `.claude-plugin/` — plugin lane manifests: `plugin.json` (the `zs` plugin) + `marketplace.json` (lists the single `zs` plugin). Note `plugin.json` carries NO `hooks` field — Claude Code auto-loads `hooks/hooks.json` from the plugin root, so a manifest reference to it causes a duplicate-hooks load error.
...
**Plugin-lane mental model — internalize this; agents repeatedly get it backwards.** The install states, most-normal first: (1) **Plugin lane, mirror-less = the norm and the goal** — a real plugin consumer has NO `.claude/skills/` mirror and, before init, **zero zskills-written files in the project**: skills, hooks, the verifier/implementer agents (plugin root `agents/`), Layer-0 timeout injection, and the managed rules all resolve or deliver from `${CLAUDE_PLUGIN_ROOT}` with no project-side writes. After the one-time `/zs:update-zskills` init the project-side footprint is exactly: one `.zskills/` umbrella entry in `.gitignore`, gitignored `.zskills/` runtime state (`init-done`, `setup-confirmed`, tracking), and the OPTIONAL `.claude/zskills-config.json` (+ its schema sibling) if the consumer accepted one at the interview. Mirror-less is PREFERRED, not a degraded mode. (2) **`/update-zskills` lane, mirrored** = the other supported single-lane state, where the `.claude/skills/` mirror IS the install. (3) **Mirrored-plugin (a `.claude/skills/` mirror present *alongside* a loaded plugin) = dual-install = NOT a supported consumer state** — it exists ONLY in this dogfooding repo. **The trap: zskills-dev IS case 3** — it carries the mirror because it is the source repo. If you are reasoning here and see `.claude/skills/`, that is the dogfooding exception you are sitting in, NOT the norm you are building for. Never treat the presence of the local mirror as evidence the plugin lane works — validate mirror-less (build the prod-stripped tree, `claude plugin marketplace add` it as a path, install) or you reproduce the "dogfood-mask" that shipped the non-functional plugin lanes in #799/#831.

**Two install lanes — a consumer picks exactly ONE.** A client is single-lane: it installs zskills via the plugin lane OR the legacy `/update-zskills` lane, never both. (Dual-install — both lanes present at once — exists only in this dogfooding repo; see "Dual-load dogfooding" below.) zskills is distributed via (1) the **plugin lane** (`claude plugin install` from a marketplace, or `claude --plugin-dir .` for dogfooding — loads `.claude-plugin/plugin.json` + `hooks/hooks.json`, resolving paths under `${CLAUDE_PLUGIN_ROOT}`) and (2) the legacy **`/update-zskills` lane** (mirrors `skills/`→`.claude/skills/`, `hooks/*.sh`→`.claude/hooks/`, registers hooks in `.claude/settings.json`, renders `CLAUDE_TEMPLATE.md`→`.claude/rules/zskills/managed.md`). We dogfood BOTH from this repo: plugin-lane via `claude --plugin-dir .` (iterate `edit → /reload-plugins → test`), legacy-lane via `/update-zskills install` (iterate `edit → /update-zskills --rerender → test`). On the plugin lane everything is **plugin-native — no session-start writes** (the old SessionStart materialiser was deleted by INSTALL_REDESIGN Phase 7): the verifier/implementer agents dispatch from the plugin's root `agents/` (bare-name dispatch, branch 1A); Layer-0 600s timeout injection is the `hooks/hooks.json` PreToolUse `inject-bash-timeout.sh` entry (T-A identity filter — absent identity ⇒ extend, `verifier`/`implementer` incl. plugin-scoped `zs:` forms ⇒ extend, foreign agents ⇒ pass through); the managed rules are delivered per-session as SessionStart `additionalContext` by `hooks/session-rules-context.sh` (R-b — zero writes; it stays silent when a legacy mirror or a project `managed.md` already delivers the rules from disk); and `hooks/session-start-greeting.sh` emits a one-line `systemMessage` greeting on every pre-init session pointing at the one-time `/zs:update-zskills` init (gitignore-first, lock-LAST `.zskills/init-done`; the `UserPromptExpansion` gate `hooks/block-unmaterialised-skill.sh` blocks state-writing skills pre-init, allow-listing read-only ones). Both lanes render the managed rules through the SAME `scripts/render-managed-rules.py` (D24) — the legacy lane to a project file at install/`--rerender` time, the plugin lane in-memory each session. See `RELEASING.md` "Dogfooding lanes" and `docs/plans/INSTALL_REDESIGN_PLAN.md` (its predecessor `docs/plans/PLUGIN_DISTRIBUTION.md` is historical). Shipped hook scripts carry a line-2 `# zskills-hook-version:` stamp the shim uses for version-skew defer; bump it when a hook's distribution version changes (`tests/test-skill-conformance.sh` gates its presence).

**Dual-load dogfooding — a permanent dogfood-repo state, not a client end-state.** Clients are single-lane (see above); this repo is permanently dual-loaded — it carries the legacy mirror because it is the source repo AND loads itself as a plugin. That is kept corruption-free by two structural pieces: (1) the D16(a) conditional-skip shim (`hooks/_lib/plugin-hook-skip-if-mirrored.sh`) sourced by every plugin-registered hook, so a plugin hook with a settings.json-registered same-basename sibling defers (basename match against the OUTERMOST `BASH_SOURCE` entry — portable across the plain-`bash` and claude-harness sourcing semantics — plus a `# zskills-hook-version:` skew guard); (2) the W6.1 hard-refuse in `/update-zskills` Step 0.7, which UNCONDITIONALLY blocks an explicit `install` from re-creating the mirror on a `detect==plugin` consumer. There is NO lane-switch machinery (deleted by INSTALL_REDESIGN Phase 7): a consumer that wants the other lane uninstalls one and installs the other (`/plugin uninstall zs@zskills`, restart, `/update-zskills install` — or the reverse), and a consumer that somehow lands on both gets a `lane.dual-unsupported` FAIL from verify-install with that same recovery. Releases ship both lanes from one dev commit: bump `plugin.json.version`, then `scripts/build-prod.sh` serves the legacy mirror and `scripts/build-plugin-release.sh` serves the plugin tree (`prod/main` + parallel `prod/<version>` tag, push gated behind `--push`); both stage the root `agents/` tree (the shared finalizer fails closed if it is missing). The plugin-mode CI lane (`.github/workflows/test.yml` `plugin-mode` job) validates the manifest + marketplace (Tier 1 mandatory; Tier 2 best-effort `claude plugin validate --strict`; Tier 3 deferred).
...
`PIPELINE_ID` to disk. `.zskills/landed` is NOT a tracking marker — it is a
separate worktree-state artifact managed by `/commit land` (via the landing
script bundled in the `commit` skill); writers moved it from the legacy
worktree-root `.landed` in the Phase-8 root-turd consolidation, and readers
dual-read (new path first, legacy root fallback) for worktrees created
before the move. The `block-unsafe-project.sh` hook fence (Tier 1) protects
every `.zskills/<subtree>/` (tracking, audit, issues, dev-server.{pid,log})
— not just `.zskills/tracking/` — so the ZSKILLS_PATH_CONFIG migration's
audit + issues subtrees are equally guarded against accidental recursive
deletion.

## Skill versioning

**Skill versioning.** Every source skill under `skills/<name>/SKILL.md` carries a `metadata.version: "YYYY.MM.DD+HHHHHH"` field — date in `America/New_York` plus a 6-char content hash. Edits to a skill body, frontmatter (other than `metadata.version` itself), or any regular file under the skill directory (mode files, references, scripts, fixtures, stubs, etc.) MUST bump this field; the date refreshes to today, the hash is recomputed via `scripts/skill-content-hash.sh`. Pure typo / formatting / whitespace edits do not require a bump (the hash naturally absorbs them since the canonical projection normalizes whitespace; see `references/skill-versioning.md` §3). Enforcement fires at three points: `warn-config-drift.sh` (Edit-time warn, fires only when the file is staged), `/commit` Phase 5 step 2.5 (commit-time hard stop), `test-skill-conformance.sh` (CI gate). The repo-level zskills version (`YYYY.MM.N`) lives in git tags and is mirrored into `.claude/zskills-config.json` by `/update-zskills`.

**PreToolUse backstop.** A fourth enforcement point — `hooks/block-stale-skill-version.sh` — fires on every `git commit` Bash invocation in any Claude Code session. It reuses `scripts/skill-version-stage-check.sh` and emits a deny envelope on drift. This closes the bare-`git commit` bypass: `/commit` step 2.5 covers `/commit` invocations, and the hook covers everything else. `git push` is NOT gated locally (see `references/skill-version-pretooluse-hook.md` D2 for rationale; CI's `test-skill-conformance.sh` is the push-time backstop). This includes commits made by the **verifier subagent** (introduced by Plan A; loaded from `.claude/agents/verifier.md` on the legacy lane, or from the plugin's root `agents/verifier.md` on the plugin lane). On the legacy lane, per Anthropic's documented design (https://code.claude.com/docs/en/sub-agents §"Hooks in subagent frontmatter"), subagent frontmatter `hooks:` declarations COMPOSE WITH (do not replace) project-level `.claude/settings.json` hooks — so the verifier's frontmatter `inject-bash-timeout.sh` AND the project's `block-unsafe-generic.sh` / `block-unsafe-project.sh` / `block-stale-skill-version.sh` ALL fire on every verifier `git commit`. On the plugin lane the same gates fire from `hooks/hooks.json` (plugin agents carry NO frontmatter `hooks:` block — the root `agents/` twins are hooks-block-stripped; Layer-0 and the block-hooks are all hooks.json-registered, so verifier commits are gated identically). **Recovery (verifier-side):** the deny envelope's `permissionDecisionReason` carries the stage-check STOP message verbatim — including the exact `bash scripts/frontmatter-set.sh <S>/SKILL.md metadata.version "$today+$hash"` command. The verifier has `Edit` and `Bash` in its tools allowlist (`tools: Read, Grep, Glob, Bash, Edit, Write` per `.claude/agents/verifier.md`) and SHOULD execute the bump inline, then re-stage and re-issue the commit. **Recovery (orchestrator-side, when a non-verifier caller hits the deny):** read the STOP message rendered in the tool-error output, run the suggested bump command, and re-issue the commit. Do NOT treat the deny as "tests failed" — it is a strict pre-flight check, not a test result.

## Verifier-cannot-run rule

**Verifier-cannot-run is a verification FAIL, not a routing decision.** When a dispatched verification subagent returns without running tests — whether because it hit the `run_in_background: true` + `Monitor`/`BashOutput` anti-pattern, exceeded the 45-minute agent timeout, or returned an empty/no-results response matching one of the stalled-string trigger phrases — the orchestrator MUST invoke the Failure Protocol (STOP, halt the pipeline, surface to the user) instead of logging a one-line note and proceeding. Inline self-verification by the orchestrator is NOT acceptable recovery — the orchestrator wrote the impl prompts and has implementer bias. The structural defense (D'' architecture, plan VERIFIER_AGENT_FIX) lives at two layers: **Layer 0 (root cause)** — `inject-bash-timeout.sh` is a `PreToolUse` hook on Bash that auto-extends every Bash call's `timeout` to 600000 ms (10 min) via the `updatedInput` envelope, so the 120s default that triggers the bg+Monitor recovery reflex never fires. It is delivered per lane: on the legacy lane as the frontmatter hook declared by `.claude/agents/{verifier,implementer}.md` (the `.claude/hooks/inject-bash-timeout.sh` copy); on the plugin lane as the `hooks/hooks.json` PreToolUse entry with the T-A identity filter (absent identity ⇒ extend — which is exactly what the identity-less frontmatter path needs from the same byte-identical script; `verifier`/`implementer` incl. `zs:`-scoped ⇒ extend; foreign agents ⇒ pass through). **Layer 3 (universal failure-protocol primitive)** — the skill-bundled `skills/update-zskills/scripts/verify-response-validate.sh` (resolved lane-portably as `$ZSKILLS_SKILLS_ROOT/update-zskills/scripts/verify-response-validate.sh`) is a script every verifier-dispatching skill pipes the verifier's response through (7-phrase stalled-string whitelist anchored to last 10 lines + 200-byte minimum-length signal); exit 1 means STOP. The verifier agent keeps the FULL tools allowlist (`Read, Grep, Glob, Bash, Edit, Write`) — structural restrictions on `Monitor`/`BashOutput` are no longer needed because Layer 0 prevents the trigger. All of these artifacts must be installed and functional. Past failures: PR #175 (skill-versioning, 2026-05-02) — every Phase 1-6 verifier dispatch hit the Monitor pattern; orchestrator did inline verification across 5 of 7 phases and committed unverified work. Issues #176, #180.
