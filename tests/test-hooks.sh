...
# Shared hooks harness (Test Suite Parallelization Phase 1a). Provides the
# PASS_COUNT/FAIL_COUNT counters, pass/fail, expect_deny/expect_allow,
# make_branch_repo, expect_*_from_repo, the project-hook fixture helpers
# (setup_project_test / teardown_project_test / setup_push_remote /
# expect_project_*), setup_project_test_on_main (folded in via the harness
# sourcing test-hooks-helpers.sh), and the absolutized hook-path globals
# (HOOK / PROJECT_HOOK / AGENTS_HOOK / WARN_HOOK). REPO_ROOT is set above, so
# the harness honours it. No test section moves in this phase — only the
# helper definitions live in the lib.
