...
echo ""
echo "=== zskills init-gate (#1121): block half-installed plugin at the resolver-source chokepoint ==="
# The gate folds into block-unsafe-generic.sh. It DENIES a Bash command that
# SOURCES zskills-resolve-config.sh ONLY when the install is NOT materialized
# (the 5 artifacts absent — detected via the materialiser sentinel
# `zskills-materialised:` on .claude/verifier.md). When materialized it ALLOWS;
# a command carrying the ZSKILLS_SKIP_INIT_GATE token (the /update-zskills cure)
# ALLOWS unconditionally; and MENTION forms (cat/grep/`[ -f ]` test line) and
# normal non-zskills commands never trip the gate. The harness controls
# artifact-presence by pointing CLAUDE_PROJECT_DIR at a temp project that either
# carries (materialized) or omits (not-materialized) a sentinel-stamped
# verifier.md.
...
# Run the generic hook with CLAUDE_PROJECT_DIR pointed at $proj.
ig_expect_deny() {
  local proj="$1" label="$2" cmd="$3" result
  result=$(echo "{\"tool_name\":\"Bash\",\"tool_input\":{\"command\":\"$cmd\"}}" | CLAUDE_PROJECT_DIR="$proj" bash "$HOOK")
  if [[ "$result" == *"permissionDecision"*"deny"* ]] && [[ "$result" == *"finished installing"* ]]; then
    pass "init-gate deny: $label"
  else
    fail "init-gate deny: $label — expected init-gate deny, got: $result"
  fi
}
ig_expect_allow() {
  local proj="$1" label="$2" cmd="$3" result
  result=$(echo "{\"tool_name\":\"Bash\",\"tool_input\":{\"command\":\"$cmd\"}}" | CLAUDE_PROJECT_DIR="$proj" bash "$HOOK")
  if [[ -z "$result" ]]; then
    pass "init-gate allow: $label"
  else
    fail "init-gate allow: $label — got unexpected output: $result"
  fi
}

IG_PROJ_NOTMAT="$(ig_make_proj not-materialized)"
IG_PROJ_MAT="$(ig_make_proj materialized)"

# The canonical legacy-lane resolver source (dual-lane prelude `else` branch).
IG_SRC_PROJ='. \"$CLAUDE_PROJECT_DIR/.claude/skills/update-zskills/scripts/zskills-resolve-config.sh\"'
# The plugin-lane resolver source.
IG_SRC_PLUGIN='. \"${CLAUDE_PLUGIN_ROOT}/skills/update-zskills/scripts/zskills-resolve-config.sh\"'

# DENY: a real resolver-source command when artifacts ABSENT (not materialized).
ig_expect_deny "$IG_PROJ_NOTMAT" "resolver-source (PROJECT_DIR form), not materialized" "$IG_SRC_PROJ"
ig_expect_deny "$IG_PROJ_NOTMAT" "resolver-source (PLUGIN_ROOT form), not materialized" "$IG_SRC_PLUGIN"
ig_expect_deny "$IG_PROJ_NOTMAT" "resolver-source after cd-chain, not materialized" "cd /tmp/x && $IG_SRC_PROJ"
ig_expect_deny "$IG_PROJ_NOTMAT" "resolver-source via 'source' keyword, not materialized" "source \\\"x/zskills-resolve-config.sh\\\""

# ALLOW: artifacts PRESENT (materialized).
ig_expect_allow "$IG_PROJ_MAT" "resolver-source, materialized" "$IG_SRC_PROJ"
ig_expect_allow "$IG_PROJ_MAT" "resolver-source (PLUGIN_ROOT form), materialized" "$IG_SRC_PLUGIN"

# ALLOW: the ZSKILLS_SKIP_INIT_GATE=1 form (the /update-zskills cure), even when
# NOT materialized.
ig_expect_allow "$IG_PROJ_NOTMAT" "ZSKILLS_SKIP_INIT_GATE=1 resolver-source, not materialized" "ZSKILLS_SKIP_INIT_GATE=1 $IG_SRC_PROJ"
ig_expect_allow "$IG_PROJ_NOTMAT" "ZSKILLS_SKIP_INIT_GATE=1 (PLUGIN_ROOT), not materialized" "ZSKILLS_SKIP_INIT_GATE=1 $IG_SRC_PLUGIN"

# ALLOW: MENTION forms (not a source) — cat / grep / `[ -f ]` test line.
ig_expect_allow "$IG_PROJ_NOTMAT" "cat ...zskills-resolve-config.sh (mention)" "cat foo/zskills-resolve-config.sh"
ig_expect_allow "$IG_PROJ_NOTMAT" "grep ... zskills-resolve-config.sh (mention)" "grep PYTHON foo/zskills-resolve-config.sh"
ig_expect_allow "$IG_PROJ_NOTMAT" "if [ -f \"...zskills-resolve-config.sh\" ] test line (mention)" "if [ -f \\\"foo/zskills-resolve-config.sh\\\" ]; then echo hi; fi"

# ALLOW: a normal non-zskills Bash command (the cheap early-exit path).
ig_expect_allow "$IG_PROJ_NOTMAT" "git status (normal command)" "git status"
ig_expect_allow "$IG_PROJ_NOTMAT" "echo hello (normal command)" "echo hello"

rm -rf "$IG_PROJ_NOTMAT"
rm -rf "$IG_PROJ_MAT"
