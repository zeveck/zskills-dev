...
| 4 — Migrate `/fix-issues pr` to `/land-pr` (drop 300s timeout special case) | 🟡 In Progress | `306e2c2` | per-issue caller-loop dispatching /land-pr inside `for issue in FIXED_ISSUES`; --issue=$ISSUE_NUM passthrough; 300s timeout dropped (default 600s); /fix-issues pr GAINS canonical fix-cycle (drift fix); 4 conformance assertions removed + 4 added; 1790/1790 tests; canary deferred (architectural) |
