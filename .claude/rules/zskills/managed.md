...
**A mid-run fire is gated, not lost.** Because cron idle is turn-level (not task-level), a fire can land in a turn-gap while a prior run of the SAME recurring skill is still executing across turns. The recurring queue-workers (`/fix-issues`, `/work-on-plans`, `/qe-audit`) guard their run entry with a session-scoped in-flight check (`create-worktree/scripts/check-inflight-batch.sh`): if THIS session already has a fresh, un-finalized run sentinel for that skill, the fire prints `<skill> run <id> in flight; skipping redundant cron pickup` and exits — the in-flight run finishes on its own turns and the next fire picks up fresh. It is session-scoped (a parallel pipeline in another session is never blocked) with a staleness escape (a crashed run never deadlocks the cron). Execute the cron fire normally as above; the skill's own guard handles the overlap. Do NOT `CronDelete` on seeing a skip message — skipping a redundant pickup is correct behavior, not a broken cron.
...
```bash
bash tests/run-all.sh    # Unit tests only -- fast, use while working
bash tests/run-all.sh    # ALL suites -- use before committing
```

**`bash tests/run-all.sh` must pass before every commit.** When reporting test
...
**NEVER modify the working tree to check if a failure is pre-existing.** No `git stash && bash tests/run-all.sh && git stash pop`, no `git checkout <old-commit>`, no temporary worktrees for comparison. These workflows are fragile -- context compaction between the modification and the restore will lose your changes. Past failure: an agent stashed changes, checked out a prior commit to verify a test failure was pre-existing, hit compaction, and never restored the working tree. If you touched code and tests fail, fix them. If you only touched content (markdown, images, etc.), don't run tests at all.
