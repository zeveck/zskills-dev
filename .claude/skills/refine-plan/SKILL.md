...
WT_PATH=$(bash "$HELPER" \
  ...
  --allow-resume \
  --purpose "refine-plan auto-worktree; isolate plan refinement from main" \
  "${TRACKING_ID}")
RC=$?
if [ "$RC" -ne 0 ]; then
  echo "ensure-worktree failed (rc=$RC) for /refine-plan" >&2
...
if [ -n "$WT_PATH" ]; then
  cd "$WT_PATH" || { echo "refine-plan: cd $WT_PATH failed" >&2; exit 1; }
...
After appending the Plan Review section, when `$TOPLEVEL` is a worktree
(not main), stage and commit `$PLAN_FILE` so the refined plan is captured
on the feature branch rather than left dirty in main. This pairs with the
...
if [ "$TOPLEVEL" != "$MAIN_ROOT" ]; then
  ...
  if [ -z "$FILE_REL" ]; then
    ...
    case "$ABS_FILE" in
      ...
      *) echo "refine-plan: cannot normalize $PLAN_FILE vs $TOPLEVEL" >&2; exit 1 ;;
    esac
  fi
  ...
  case "$FILE_REL" in
    /*|../*) echo "refine-plan: $PLAN_FILE is outside worktree $TOPLEVEL" >&2; exit 1 ;;
  esac

  git -C "$TOPLEVEL" add "$FILE_REL"
  STAGED=$(git -C "$TOPLEVEL" diff --cached --name-only)
  if [ "$STAGED" != "$FILE_REL" ]; then
    echo "refine-plan: unexpected staged set: $STAGED (expected $FILE_REL only)" >&2
    exit 1
  fi
  if [ -n "${CLAUDE_PLUGIN_ROOT:-}" ] && [ -f "${CLAUDE_PLUGIN_ROOT}/skills/update-zskills/scripts/zskills-resolve-config.sh" ]; then
    . "${CLAUDE_PLUGIN_ROOT}/skills/update-zskills/scripts/zskills-resolve-config.sh"
