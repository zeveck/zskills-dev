# /draft-tests — Phases 1-4 (parse, language detect, draft, adversarial review)

This file is the main pipeline for `/draft-tests`. The orchestrator's
[SKILL.md](../SKILL.md) router dispatches here after preflight, worktree
preamble, and argument parsing complete.

Variable contract — the SKILL.md preamble has already set: `$PLAN_FILE`,
`$TRACKING_ID`, `$AUTO_FLAG`, `$MAIN_ROOT`, `$TOPLEVEL`, `$WT_PATH`,
`$ZSKILLS_PATHS_ROOT`, `$ARGUMENTS` (with `auto` consumed). Phases 1-4
below construct `$PIPELINE_ID`, `$PARSED_STATE`, `$DETECT_STATE`,
`$SPECS_FILE`, and the per-round artifacts; Phase 5
([modes/backfill.md](backfill.md)) and Phase 6
([modes/land.md](land.md)) read them downstream.
...
**Read [../references/design-constraints.md](../references/design-constraints.md)**
for the cross-phase invariants that apply across Phases 1-4: the
checksum gate, AC-ID-as-only-permitted-edit rule, cross-skill script
invocation form, no-external-JSON-tooling rule, tracking marker scheme,
and persisted parsed-state contract.
...
**Read [../references/test-spec-format.md](../references/test-spec-format.md)**
for the canonical one-line bullet form, the multi-line expansion form,
and the anti-pattern list that goes verbatim into the drafter prompt.
Both this phase (the drafter) and Phase 4 (the reviewer / devil's
advocate / refiner) cite this file as the single source of truth for
spec format.
