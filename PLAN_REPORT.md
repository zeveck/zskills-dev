...
|--------|--------|--------|
| [plan-pr-landing-unification.md](reports/plan-pr-landing-unification.md) | 6/7 | **In progress** — Phases 1A-4 landed (PRs #159-#163). Phase 5 (this PR): /quickfix migrated → /land-pr; Phase 7 PR creation block replaced with caller-loop; "Fire-and-forget" prose REMOVED ("triage→review→commit→push→PR→CI poll→fix cycle" lifecycle); pre-PR triage + plan-review gates preserved; fulfillment-marker model preserved (no .landed); 3 conformance assertions added; tests/test-quickfix.sh adjusted (regex tightened — strengthened, Case 43b structural assertions added); 1794 tests. **5/5 PR-landing call sites now dispatch /land-pr.** Phase 6 (drift-prevention conformance + canary) is the final phase. |
| [plan-draft-tests-skill-plan.md](reports/plan-draft-tests-skill-plan.md) | 6/6 | **Complete** — all 6 phases landed on `feat/draft-tests-skill-plan` (PRs #124, #135, #136, #137, #139, plus this final): skeleton + checksum gate, language detection + no-test-setup path, drafting agent + spec format, adversarial QE-persona review loop with orchestrator-judgment convergence and merged-candidate coverage-floor pre-check, backfill mechanics with broad-form heading rules and fenced-code-block awareness and parsed-state enrollment in coverage floor, conformance + worked example + mirror; 1670/1670 tests; landing via PR squash |
...
| [plan-zskills-monitor-plan.md](reports/plan-zskills-monitor-plan.md) | 6/9 | **In progress** — Phases 1+2+3+4+5+6 done (CLI + /plans work retired + flock + aggregation lib + HTTP server + read-only UI); 1111/1111 tests; Phases 7-9 remaining |
...
| [plan-restore-chunked-execution.md](reports/plan-restore-chunked-execution.md) | 5 | Landed |
