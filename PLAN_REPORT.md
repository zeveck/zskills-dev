...
|--------|--------|--------|
| [plan-pr-landing-unification.md](reports/plan-pr-landing-unification.md) | 5/7 | **In progress** — Phases 1A-3 landed (PRs #159-#162). Phase 4 (this PR): /fix-issues pr migrated → /land-pr per-issue caller-loop; --issue=$ISSUE_NUM passthrough; 300s timeout dropped (default 600s); /fix-issues pr GAINS canonical fix-cycle (drift fix); 1790 tests. 4/4 PR-landing call sites (/run-plan, /commit pr, /do pr, /fix-issues pr) now dispatch /land-pr. Phase 5 (/quickfix migration) remains. |
