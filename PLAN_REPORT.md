...
|--------|--------|--------|
| [plan-pr-landing-unification.md](reports/plan-pr-landing-unification.md) | 4/7 | **In progress** — Phase 1A (PR #159 `1c6c143`): /land-pr skill + 4 scripts + 2 refs. Phase 1B (PR #160 `8d9bc13`): failure-modes + mocks + 23 tests + 37 conformance. Phase 2 (PR #161 `e28c7f6`): /run-plan PR mode → /land-pr dispatch; 0 inline gh pr create/merge/checks --watch; 1792 tests. Phase 3 (this PR): /commit pr + /do pr migrated → /land-pr (drift fix: GAIN fix-cycle); skills/commit/scripts/poll-ci.sh deleted (orphan); /do/SKILL.md "report-only CI" key-rule REMOVED; 1790 tests. Phase 2 canary verified-in-action (this phase used new /run-plan code). |
