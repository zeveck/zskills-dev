...
YOU should not be the verifier without checking the artifacts.
...
everything inline when it isn't.

Do not produce a verification report based on what you remember doing.
Read actual diffs. Run actual tests. The ENTIRE POINT of `/verify-changes`
is fresh eyes on the actual state of the code, not a rubber stamp.
...
4. **For each `User Verify: NEEDED` item, include verification instructions:**
   ...
   - **Dev server URL with actual port number** — run `bash scripts/port.sh`
     to get the port, then include the full clickable URL:
     `http://localhost:<PORT>/`. If the dev server isn't running, START IT
     first (`npm start &` or equivalent) so the URL actually works when
     the user clicks it. If working in a worktree, the port differs from
     main — use the worktree-specific port.
   ...
   A relative path to a report file is not enough — include the live URL.
