...
**before push**, captured in the squash. Also regen `$ZSKILLS_AUDIT_DIR/plan-{slug}.md`
and `$ZSKILLS_AUDIT_DIR/PLAN_REPORT.md` here if they need post-landing
updates (strip `[UNFINALIZED]`, add merge note).

Resolve `$ZSKILLS_AUDIT_DIR` via the path-config helper anchored on
`$WORKTREE_PATH` (PR mode bookkeeping commits inside the worktree):
...
git add <plan-file> ["$ZSKILLS_AUDIT_DIR/plan-{slug}.md" "$ZSKILLS_AUDIT_DIR/PLAN_REPORT.md"]
...
**Report:** See \`\$ZSKILLS_AUDIT_DIR/plan-${PLAN_SLUG}.md\` for details.
