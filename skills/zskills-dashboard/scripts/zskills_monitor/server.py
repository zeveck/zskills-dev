...
def _load_zskills_config(main_root: pathlib.Path) -> Dict[str, Any]:
    ...
    CASCADE v2 (ENFORCEMENT_V2 Phase 4): the user tier's `execution` block is
    dropped from the blanket merge, then SELECTIVELY re-merged for exactly the
    five #1159-scoped keys (3 plain-cascade workflow keys + 2 raise-only
    safety floors) via `_merge_execution_cascade_v2`. Every other execution.*
    key (and agents.* apart from the min_model floor) stays project-tier-only.
    ...
    project = _read_config(main_root)
