...
created: 2026-05-21
status: "complete"
completed: "2026-05-28T22:01:34Z"
---
...
> **SUPERSEDED NOTE (INSTALL_REDESIGN, Phase 9):** The plugin-lane install model this plan built was redesigned by `docs/plans/INSTALL_REDESIGN_PLAN.md` (2026-06): the SessionStart materialiser, the bidirectional lane-switch tooling (`--switch-install-path`, `switch-install-path.sh`, the lane lock, the switch-in-progress marker), the D20 materialised-sentinel convention, and the D27 dual-install probe described below were all **deleted**. The plugin lane is now zero-write by default with an explicit one-time `/zs:update-zskills` init; agents/Layer-0/rules are plugin-native (root `agents/`, a hooks.json PreToolUse timeout entry, SessionStart `additionalContext` rules); and lane switching is "uninstall one, install the other". The D16 conditional-skip shim survives. The body below is the historical record of the original dual-path build — read INSTALL_REDESIGN_PLAN.md for the current model.
...
- **(a) Plugin hooks.json conditional skip — basename-match against settings.json, with version-skew guard.** Hook entries per `/tmp/research-plugin-schema.md` §5 lines 215-231 are anonymous `{ "type": "command", "command": "<string>" }` records — there is NO `name` field. The shim's matching mechanism is therefore basename-based, defined precisely as:
  ...
  2. The shim computes `MY_BASENAME=$(basename "${BASH_SOURCE[${#BASH_SOURCE[@]}-1]}")` — the basename of the OUTERMOST `BASH_SOURCE` stack entry, i.e. the hook script the harness invoked. **Correction (2026-05-28, Phase 5 W5.7):** earlier drafts used `${BASH_SOURCE[0]}` with an empirical note that `[0]` is the sourcing hook under `claude` 2.1.149. That holds only under claude's no-extra-frame sourcing model (stack depth 1, so `[0]==[last]`); under a plain `bash <hook>` invocation that `source`s the shim, `source` DOES push a frame, so `${BASH_SOURCE[0]}` is the shim's own path and the basename never matches — silently no-op'ing the double-fire guard everywhere except that one claude build. The outermost-frame form `${BASH_SOURCE[${#BASH_SOURCE[@]}-1]}` resolves to the hook in BOTH models (claude depth-1 `[0]==[last]`; plain-bash `[last]`=hook) and is the shipped form, gated by `tests/test-plugin-hook-skip-on-double-register.sh`.
...
- **(b) `.gitignore` guidance — scoped to plugin-lane consumers (F-R1-16).** `docs/guides/PLUGIN_INSTALL.md` tells PLUGIN-LANE consumers to add the 5 materialised paths to `.gitignore` — they're plugin-managed. `/update-zskills`-lane consumers receive different guidance: leave the paths tracked (they ARE the install state).
...
Both directions enforce the lock-LAST contract per CLAUDE.md `## Migration scripts`. Both directions have an Abort/Rollback path documented in `docs/guides/PLUGIN_MIGRATION.md`. Both directions have a fixture-dogfood acceptance test in `tests/test-switch-install-path.sh`.
...
`docs/guides/PLUGIN_INSTALL.md` carries the full tradeoff matrix. README points to it.
...
| 2 — SessionStart materialiser (5 artifacts) + dual-install detection + renderer equivalence | ✅ | a0187a3 | session-start-materialise.sh + detect-install-state.sh (D27) + managed_rules_substitution.py/render-managed-rules.py (D24) + Step B/D port; 8 new tests. (Hash was f750100 pre-rebase. The "2 pre-existing failures" claimed at the time were FALSE — worktree-staleness artifacts, fixed in Phase 3; suite is 6167/6167 green.) |
| 3 — Dual-path recognition + cron-fire path-aware rules + script-path fallback in all resolver-sourcing + legacy-script-path sites (re-derive; was 151/228) + `migrate-crons` skill | ✅ | 9d22c48 | 151 resolver sites + legacy refs dual-pathed (source+mirror); cron-fire OR-match `/`+`/zs:` permanent + dual SKILL.md path; managed.md re-rendered; migrate-crons skill; 24 dirs version-bumped; 3 new tests. Rebased onto origin/main (#766). Suite 6167/6167, 0 failed. |
| 4 — Conformance test surface (no retirements; 16 new tests stabilise; 2 restructured) | ✅ | e7575bb | 0 retired (10 update-zskills/drift tests confirmed green+present); W4.2(b) materialiser-presence tripwire added; conformance dual-path + version-enforcement trio confirmed already-satisfied (no churn); 5 consecutive iterations 6173/6173, 0 failed; forbidden-literals clean |
| 5 — Dual-path hardening: plugin-mode CI lane, bidirectional `--switch-install-path`, hook double-fire prevention, `build-plugin-release.sh` | ✅ | 63ef305 | plugin-mode CI (3 tiers); switch-install-path.sh bidirectional (lock-LAST, proven file-removal-safe) + migrate-strip-settings.py; build-plugin-release.sh (no self-delete, --push-gated, D3/D4); hook-double-fire testbed + **shim BASH_SOURCE fix** (W5.7 caught a real Phase-1 bug); plugin.json 2026.05.1 lockstep; suite 6249/6249. Release PUSH is human-gated (not done). |
| 6 — Marketplace activation, prod-tag release flow, README + dual-path docs | ✅ | 05aebeb | docs/guides/PLUGIN_INSTALL.md (dual-lane onboarding, D26 default + tradeoff matrix, D1 pin idiom, D20(b) gitignore, Known-tradeoffs/bare-slash gap); README both lanes + link; PLUGIN_MIGRATION.md confirmed (Phase 5); #432 already closed; MW-EXAMPLE absent. **Marketplace activation + prod-tag PUSH are human-gated — documented, not executed.** |
...
| (2) `prod/main` ref + marketplace cache semantics | 5 W5.5 + 6 W6.2 | If `/plugin marketplace update` doesn't fetch latest reliably, consumers stay on stale plugin. | Parallel `prod/<version>` tag scheme (D1) gives consumers a stable pin; docs/guides/PLUGIN_INSTALL.md documents the workaround. |
| (3) `Agent` / `CronCreate` matcher syntax in `hooks.json`; relative-path marketplace source; `dependencies` semantics | 1 W1.6 | If `claude plugin validate --strict` rejects the matcher, fall back to `PreToolUse` with `tools: [...]`. If the relative-path source for `zsbd` is rejected, fall back to `git-subdir` (both documented per `/tmp/research-plugin-schema.md` §3). If `dependencies` on `zsbd` is rejected (research-deferred per §15 line 574), accept orphan-install gap and document in `docs/guides/PLUGIN_INSTALL.md`. **Empirical update (2026-05-22):** ran `claude plugin validate --strict` on a fixture plugin with `Bash`, `Agent`, `CronCreate`, `Edit|Write`, `SessionStart`, `Notification`, and several other matchers — ALL passed. The matcher catalog published in the Claude Code docs is incomplete (`CronCreate` isn't enumerated), but the validator and runtime both accept it. Today's production `.claude/settings.json` uses `CronCreate` with confirmed firing behavior. Net effect: Phase 1 W1.6's `claude plugin validate --strict` gate catches structural schema errors but does NOT enforce matcher correctness. Empirical runtime confirmation moves to Phase 2 dogfood as a known-low-risk verification. (Note: zskills has dropped `NotebookEdit` from its `block-main-edits` matcher — the matcher is now `Edit|Write` — so any residual ambiguity around that specific matcher token is moot for our hooks.) | Phase 1 Abort/Rollback path. |
...
- **Bare-slash prose in skill bodies is NOT rewritten** — pivot point 4. Plugin-lane consumers see bare slashes in prose; acceptable per Risk Register gap (8) and `docs/guides/PLUGIN_INSTALL.md` "Known tradeoffs."
...
- [ ] **W6.1 — Consumer-onboarding documentation (`docs/guides/PLUGIN_INSTALL.md`).** Documents BOTH paths under a side-by-side comparison. Includes:
...
- [ ] **W6.2 — Dual-path migration runbook (`docs/guides/PLUGIN_MIGRATION.md`).** Documents the bidirectional `--switch-install-path` tool (D25). Renamed from round-3's one-way framing ("migrating away from `/update-zskills`") to "switching install paths (optional)." Covers Abort/Rollback for both directions.
...
- [ ] **W6.6 — README.md updates.** Document both install paths at top-level. Point at `docs/guides/PLUGIN_INSTALL.md` for the full comparison. Default recommendation per D26.

### Tests

- [ ] `docs/guides/PLUGIN_INSTALL.md` and `docs/guides/PLUGIN_MIGRATION.md` exist.
...
4. **No bulk slash-prefix rewrite.** Phase 3 W3.2 (the 1,621-hit prose rewrite) deleted. F-R1-2. Bare slashes in skill-body prose stay; documented as a plugin-lane UX rough edge in `docs/guides/PLUGIN_INSTALL.md`. F-DA1-1 Option Y locked.
