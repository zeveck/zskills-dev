...
### #1150 — Post-redesign polish batch: prose residues, rules silent-fail nudge, version clobber guard, guide drift

**Labels:** (none) | **Verdict:** FULLY SPECIFIED — consolidated review findings
**Problem.** Five small, none state-corrupting: (1) BD prose residues ×2 (research-and-go/SKILL.md:52 worked example; run-plan/modes/execute-phase.md:713 /add-block anecdote); (2) session-rules-context.sh render failure exits 0 silently — no rules, no signal; (3) update-arm version refresh can clobber a good version: line with empty when resolve-repo-version.sh fails; (4) install-guide drift (scope flag never named + headless default user; ~15KB stale vs ~39.5KB measured; headless gate-block renders as empty success — doc note); (5) optional NITs (non-git duplicate gitignore appends; $$ tmp collisions → $BASHPID; project_name-only seed classification).
**Fix outline.** Per the issue body: one-line prose swaps + version bumps + mirrors for (1); a systemMessage nudge on render failure for (2); preserve-prior-value-when-empty in the update-arm writer for (3); installing-zskills.md edits for (4); (5) at implementer discretion if cheap (each is 1-3 lines; $BASHPID swap + duplicate-append guard are safe; skip the seed-matcher NIT — design-adjacent).
**Complexity:** M (breadth, not depth). **Action now:** fix-issues in-batch (user-pinned).
