...
# Contract: read PIDs from .zskills/dev-server.pid (one per line) and
# SIGTERM each. Pair with scripts/start-dev.sh which writes
# to .zskills/dev-server.pid on start.
#
...
echo "stop-dev.sh: not configured. Edit scripts/stop-dev.sh with your dev-stop command (read PIDs from .zskills/dev-server.pid; kill -TERM each). See .claude/skills/update-zskills/references/stub-callouts.md." >&2
exit 1
